...
const zh: Record<string, string> = {
  ...
  settings_proxy_update_on: '代理更新已开启',
  settings_proxy_update_off: '代理更新已关闭',
  ...
  settings_dsh_mem_limit: 'node堆内存上限',
  settings_dsh_mem_mb: '(MB)',
  settings_dsh_mem_hint: '仅在 node 堆内存不足时适当增大',
  settings_dsh_mem_warn: '堆内存上限较低，dsh 可能无法稳定运行',
  settings_dsh_mem_too_low: '堆内存上限过低，请增大分配',
  settings_dsh_mem_auto: '自动设置',
  settings_dsh_mem_auto_hint: '当前自动分配的堆内存上限，如 dsh 内存占用接近上限，建议手动增大上限避免频繁 GC',
  settings_browser_compat: '浏览器兼容模式',
  settings_browser_compat_hint: '使用 Firefox / 苹果设备（WebKit 内核）的用户建议开启：修复会话历史无法加载，以及苹果设备上模型与推理等级无法切换。开启后建议清除浏览器缓存并刷新页面',
  ...
  saved_mem_restart: '配置已保存，堆内存上限已变更，请重启 dsh 服务使配置生效',
  ...
  confirm_ok: '确定',
  confirm_cancel: '取消',
  // 弹窗右上角 X 的悬停提示 / 无障碍标签（见 components/DialogCloseButton.vue）
  dialog_close: '关闭',
  confirm_delete: '删除',
  ...
  plugin_uninstall: '卸载',
  // 插件名点击复制（版本号刻意不可复制）
  plugin_copy_name: '点击复制插件名称',
  plugin_name_copied: '已复制插件名称',
  plugin_removing: '卸载中…',
...
const en: Record<string, string> = {
  ...
  settings_proxy_update_on: 'Proxy updates enabled',
  settings_proxy_update_off: 'Proxy updates disabled',
  ...
  settings_dsh_mem_limit: 'Node heap limit',
  settings_dsh_mem_mb: '(MB)',
  settings_dsh_mem_hint: 'Increase only when node runs out of heap memory',
  settings_dsh_mem_warn: 'Heap limit is low — dsh may become unstable',
  settings_dsh_mem_too_low: 'Heap limit is too low — increase it',
  settings_dsh_mem_auto: 'Auto',
  settings_dsh_mem_auto_hint: 'Currently allocated automatically. If dsh memory usage approaches this limit, raise it manually to avoid frequent GC',
  settings_browser_compat: 'Browser compatibility mode',
  settings_browser_compat_hint: 'Recommended for Firefox / Apple (WebKit) devices — fixes session history failing to load, and the model / reasoning-effort selector not switching on Apple devices. After enabling, clear the browser cache and reload the page',
  ...
  saved_mem_restart: 'Saved. Heap limit changed — restart dsh to apply.',
  ...
  plugin_copy_name: 'Click to copy the plugin name',
  plugin_name_copied: 'Plugin name copied',
