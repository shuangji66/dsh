...
const zh: Record<string, string> = {
  ...
  settings_proxy_port_hint: '0.1.7-alpha.1 起支持飞牛网关访问，现飞牛入口已切换至网关访问，此设置为外部端口访问',
  ...
  settings_auth_gateway_hint: '从飞牛网关访问会跳过登录鉴权（网关已完成登录认证）；端口访问建议开启鉴权，否则同一网络内可直接访问 dsh 服务。',
  ...
  settings_browser_compat_hint: '使用 Firefox / 苹果设备（Safari）的用户建议开启，修复会话历史无法加载的问题；其他浏览器开启无副作用。开启后需清除浏览器缓存并刷新页面',
  settings_browser_compat_saved: '已保存。请清除浏览器缓存并刷新页面使其生效（或强制刷新 Ctrl+Shift+R）',
...
const en: Record<string, string> = {
  ...
  settings_proxy_port_hint: 'fnOS gateway access is supported since 0.1.7-alpha.1; the fnOS entry now goes through the gateway, so this setting is for external port access',
  ...
  settings_auth_gateway_hint: 'Access through the fnOS gateway skips the login check (the gateway already authenticates you). Keep auth enabled for port access, or anyone on the same network can reach the dsh service directly.',
  ...
  settings_browser_compat_hint: 'Recommended for Firefox / Apple (Safari) devices — fixes session history failing to load. Harmless in other browsers. Clear the browser cache and reload the page after enabling.',
  settings_browser_compat_saved: 'Saved. Clear the browser cache and reload the page to apply (or hard-reload with Ctrl+Shift+R).',
