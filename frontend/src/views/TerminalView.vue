...
<template>
  ...
  <div class="flex flex-col h-[calc(100dvh-56px)] md:h-[100dvh]">
    ...
    <!-- 移动端功能键栏 (仅在移动端显示) -->
    <div class="flex md:hidden items-center justify-around gap-1 px-3 py-2.5 pb-4 mb-1 bg-bg dark:bg-[#0B0B0F] border-t border-line dark:border-[#2A2A32] flex-wrap"
      style="padding-bottom: calc(env(safe-area-inset-bottom, 0px) + 10px);">
      <button class="px-2.5 py-1 text-xs font-medium rounded-full transition"
        :class="ctrlPressed ? 'bg-brand text-white' : 'bg-black/5 dark:bg-white/10 text-ink-soft dark:text-[#A6A6AD]'"
        @click="toggleModifier('ctrl')">Ctrl</button>
      <button class="px-2.5 py-1 text-xs font-medium rounded-full transition"
        :class="altPressed ? 'bg-brand text-white' : 'bg-black/5 dark:bg-white/10 text-ink-soft dark:text-[#A6A6AD]'"
        @click="toggleModifier('alt')">Alt</button>
      <button class="px-2.5 py-1 text-xs font-medium rounded-full transition"
        :class="shiftPressed ? 'bg-brand text-white' : 'bg-black/5 dark:bg-white/10 text-ink-soft dark:text-[#A6A6AD]'"
